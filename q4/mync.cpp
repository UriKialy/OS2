...
int start_tcp_server(const string &port)
{
    ...
    { // set socket options if error handle it
    ...
    { // bind the socket if error handle it
    ...
    { // listen on the socket if error handle it
...
int start_tcp_client(const string &hostname, const string &port)
{
    ...
    { // connect to the server if error handle it
...
{
    int server_sock = socket(AF_INET, SOCK_DGRAM, 0); // create a socket if error handle it
    if (server_sock < 0)
    { // handle error
        perror("Error creating socket");
        exit(EXIT_FAILURE);
...
int start_udp_client(const string &hostname, const string &port)
{
    int client_sock = socket(AF_INET, SOCK_DGRAM, 0); // create a socket if error handle it
    if (client_sock < 0)
    {
        perror("Error creating socket");
        exit(EXIT_FAILURE);
    ...
    // Note: No need to connect for UDP client
...
int main(int argc, char *argv[])
{
    ...
    signal(SIGALRM, handle_timeout); // Handle alarm signal for timeout
    int opt;
    char *program = nullptr;
    string input_redirect, output_redirect, timeout;
    bool udp_server = false;
    bool udp_client = false;
    ...
    while ((opt = getopt(argc, argv, "e:i:o:b:t:")) != -1)
    {
        ...
        case 'i':
            ...
            if (input_redirect.substr(0, 4) == "UDPS")
            {
                udp_server = true;
                // Extract port from input_redirect
                string port = input_redirect.substr(4);
                cout << "Starting UDP server on port " << port << endl;
                int server_sock = start_udp_server(port);
                // Set alarm for timeout
                if (!timeout.empty())
                {
                    alarm(stoi(timeout));
                }
                // handle_client_input for UDP server
        ...
        case 'o':
            ...
            if (output_redirect.substr(0, 4) == "UDPC")
            {
                // Extract hostname and port from output_redirect
                ...
                if (comma_pos != string::npos)
                {
                    ...
                    cout << "Starting UDP client with " << hostname << " and port " << port << endl;
                    int client_sock = start_udp_client(hostname, port);
                    // handle_client_output for UDP client
    ...
    if (pid == 0)
    { // Child process
        if (udp_server && udp_client)
        {
            cerr << "Cannot have both UDP server and client in the same process" << endl;
            return EXIT_FAILURE;
        }
    ...
    { // Parent process
        ...
        if (!timeout.empty())
        {
            ...
            if (running)
            {
                cout << "Timeout reached. Killing child process..." << endl;
                kill(pid, SIGKILL);
            }
        }
        else
        {
            ...
            while (running && waitpid(pid, &status, 0) == 0)
            {
                sleep(1);
            }
        }
